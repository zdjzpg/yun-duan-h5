...
function generateSeatsForVenue001(): VenueSeat[] {
  ...
  for (let row = 1; row <= 5; row++) {
    for (let seat = 1; seat <= 20; seat++) {
      ...
      const disabledReason = isEquipmentDisabled ? 'equipment' : isOtherDisabled ? 'other' : undefined;
  ...
  for (let row = 1; row <= 15; row++) {
    for (let seat = 1; seat <= 20; seat++) {
      ...
      const disabledReason = isMaintenanceDisabled ? 'maintenance' : isCornerDisabled ? 'other' : undefined;
  ...
  for (let row = 1; row <= 20; row++) {
    for (let seat = 1; seat <= 25; seat++) {
      ...
      const disabledReason = isEquipmentDisabled ? 'equipment' : isFireExitDisabled ? 'other' : undefined;
...
export function setupVenueListMock(mock: MockAdapter) {
  mock.onGet("/theater/venues").reply((config) => {
    const params: VenueListRequest = config.params || {};
      ...
      totalCapacity:
        request.capacityType === "free_seating"
          ...
          : request.capacityType === "zone_capacity"
            ? (request as any).zones.reduce(
                (sum: number, zone: any) => sum + zone.capacity,
                0
              )
...
export function setupVenueDetailMock(mock: MockAdapter) {
  mock.onGet(/\/theater\/venues\/[\w-]+$/).reply((config) => {
...
export function setupUpdateVenueMock(mock: MockAdapter) {
  mock.onPut(/\/theater\/venues\/[\w-]+/).reply((config) => {
    const id = config.url?.split("/").pop();
    ...
    if (request.name) {
      const duplicateName = mockVenues.find(
        (v) => v.id !== id && v.name === request.name
      );
      if (duplicateName) {
        const response: ApiResponse<null> = {
          code: ApiResponseCode.INVALID_PARAMS,
...
export function setupUpdateVenueStatusMock(mock: MockAdapter) {
  mock.onPatch(/\/theater\/venues\/[\w-]+\/status/).reply((config) => {
    const id = config.url?.split("/")[3];
    const request: UpdateVenueStatusRequest = JSON.parse(config.data);
    const venueIndex = mockVenues.findIndex((v) => v.id === id);
...
export function setupDeleteVenueMock(mock: MockAdapter) {
  mock.onDelete(/\/theater\/venues\/[\w-]+/).reply((config) => {
    const id = config.url?.split("/").pop();
