...
- **StoreSelectorModal**（`src/components/store/StoreSelectorModal.vue`）：门店选择弹窗，支持树形多选，“假输入框”触发的筛选面板（关键字 / 门店类型 / 门店标签），以及基于当前筛选结果的“全选”逻辑，示例见 `src/views/Demo/StoreSelectorDemo.vue`。
